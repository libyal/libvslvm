...
/* TODO determine how the LVM raid chunk size is determined, 64k seems to be pretty standard */
	     ...
	     segment_index++ )
	{
		...
		//get the type
		int  segment_type = LVM_LV_TYPE_UNKNOWN;
		if (libvslvm_segment_get_type(segment, &segment_type, error) != 1)
		{
			libcerror_error_set(
				error,
				LIBCERROR_ERROR_DOMAIN_RUNTIME,
				LIBCERROR_RUNTIME_ERROR_GET_FAILED,
				"%s: unable to retrieve thesegment type",
				function);

			goto on_error;
		}
		if (LVM_LV_TYPE_JBOD == segment_type)
		{
				...
				stripe_index++)
			{
					...
					error) != 1)
				{
					libcerror_error_set(
						...
						LIBCERROR_ERROR_DOMAIN_RUNTIME,
						LIBCERROR_RUNTIME_ERROR_FINALIZE_FAILED,
						"%s: unable to free physical volume: %s.",
						function,
						physical_volume_name);
...
int libvslvm_logical_volume_compare_by_name(
	libvslvm_logical_volume_t * logical_volume, 
	const char * name, 
	size_t name_length, 
	libcerror_error_t ** error)
{
	libvslvm_internal_logical_volume_t *internal_logical_volume = NULL;
	static char *function = "libvslvm_logical_volume_compare_by_name";
	...
	if ((internal_logical_volume->logical_volume_values->name_size == name_length )
		&& (memory_compare(
			name,
			internal_logical_volume->logical_volume_values->name,
			internal_logical_volume->logical_volume_values->name_size) == 0))
	{
		return(1);
	}
	return(0);
