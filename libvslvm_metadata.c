...
#endif
/* TODO */
			/*++ liubo 20190306 raid lvm*/				
				uint32_t type = 0;
				if (narrow_string_compare(value, STRIPED, value_length) == 0)
				{
					type = LVM_LV_TYPE_JBOD;
				}
				else if (narrow_string_compare(value, RAID1, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID1;
				}
				else if (narrow_string_compare(value, RAID4, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID4;
				}
				else if (narrow_string_compare(value, RAID5, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID5_LS;
				}
				else if (narrow_string_compare(value, RAID5_LA, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID5_LA;
				}
				else if (narrow_string_compare(value, RAID5_RA, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID5_RA;
				}
				else if (narrow_string_compare(value, RAID5_LS, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID5_LS;
				}
				else if (narrow_string_compare(value, RAID5_RS, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID5_RS;
				}
				else if (narrow_string_compare(value, RAID6, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID6_ZR;
				}
				else if (narrow_string_compare(value, RAID6_ZR, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID6_ZR;
				}
				else if (narrow_string_compare(value, RAID6_NR, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID6_NR;
				}
				else if (narrow_string_compare(value, RAID6_NC, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID6_NC;
				}
				else if (narrow_string_compare(value, RAID10, value_length) == 0)
				{
					type = LVM_LV_TYPE_RAID10;
				}
				else
				{
					type = LVM_LV_TYPE_UNKNOWN;
				}
				
				((libvslvm_internal_segment_t *)segment)->type  = type;
				...
				11) == 0)
			{
					...
					error) != 1)
				{
					libcerror_error_set(
						...
						LIBCERROR_ERROR_DOMAIN_MEMORY,
						LIBCERROR_MEMORY_ERROR_SET_FAILED,
						"%s: unable to set stripe_size.",
						function);

					goto on_error;
				}
				((libvslvm_internal_segment_t *)segment)->stripe_size = value_64bit;
			}
		}
		/*20190306	Lliubo raid lvm
		*/
			...
			5) == 0)
		{
			...
			if ((value_length != 1)
				|| (value[0] != '[')
				|| (libvslvm_metadata_read_raids_list(
					metadata,
					segment,
					lines,
					number_of_lines,
...
/* Reads the raids list
 * Returns the 1 if succesful or -1 on error
 */
int libvslvm_metadata_read_raids_list(
	libvslvm_metadata_t *metadata,
	libvslvm_segment_t *segment,
	libcsplit_narrow_split_string_t *lines,
	int number_of_lines,
...
{
	libvslvm_internal_volume_group_t *internal_volume_group = NULL;
	libvslvm_raid_t *raid = NULL;
	char *line_string_segment = NULL;
	char *image_name = NULL; // image_name
	char *meta_name = NULL; // meta name
	static char *function = "libvslvm_metadata_read_raids_list";
	size_t line_string_segment_index = 0;
	size_t line_string_segment_size = 0;
	size_t meta_length = 0; // meta size
	size_t image_length = 0; // image size
	...
	*line_index += 1;
	...
	while (*line_index < number_of_lines)
	{
		...
		/* Determine the value identifier
		 */
		meta_name = &(line_string_segment[line_string_segment_index]);
		meta_length = 0;
	...
	//	line_string_segment_index++;
		...
		/* Determine the value
		 */
		image_name = &(line_string_segment[line_string_segment_index]);
		image_length = 0;	
		...
		*line_index += 1;
		...
		if (libvslvm_raid_set_meta_name(
			raid,
			meta_name,
			meta_length + 1,
			error) != 1)
		...
		if (libvslvm_raid_set_image_name(
			raid,
			image_name,
			image_length + 1,
			error) != 1)
		...
		if (libvslvm_segment_append_raid(
			segment,
			raid,
			error) != 1)
		{
			libcerror_error_set(
				...
				LIBCERROR_ERROR_DOMAIN_RUNTIME,
				LIBCERROR_RUNTIME_ERROR_APPEND_FAILED,
				"%s: unable to append raid to segment.",
				function);
...
on_error:
	if (raid != NULL)
	{
		libvslvm_internal_raid_free(
			(libvslvm_internal_raid_t **)&raid,
			NULL);
	}
	return(-1);
}