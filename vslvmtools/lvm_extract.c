...
static void lvm_extract_to_extended_path(
    wchar_t       *out,
    size_t         out_count,
    const wchar_t *path )
{
    wchar_t full[ LVM_EXTRACT_PATH_MAX ];
    DWORD   len;
    ...
    len = GetFullPathNameW( path, LVM_EXTRACT_PATH_MAX, full, NULL );
    if( len > 0 && len < LVM_EXTRACT_PATH_MAX )
    {
    ...
    int           *file_count )
{
    ...
    wchar_t          src_path[ LVM_EXTRACT_PATH_MAX ];
...
{
    wchar_t          search_path[ LVM_EXTRACT_PATH_MAX ];
    wchar_t          child_path[ LVM_EXTRACT_PATH_MAX ];
    WIN32_FIND_DATAW fd;
    HANDLE           hFind;

    _snwprintf( search_path, LVM_EXTRACT_PATH_MAX, L"%s\\*", path );
    hFind = FindFirstFileW( search_path, &fd );
    if( hFind != INVALID_HANDLE_VALUE )
    {
        do
        {
            ...
            _snwprintf( child_path, LVM_EXTRACT_PATH_MAX, L"%s\\%s", path, fd.cFileName );
            if( fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
            {
    ...
    wchar_t *argv[] )
{
    ...
    lvm_extract_log( L"Launched (PID %u) - waiting for filesystem to become ready...",
                     pi.dwProcessId );
    ...
    int     file_count = 0;
    wchar_t mount_point_ext[ LVM_EXTRACT_PATH_MAX ];
    wchar_t output_dir_ext[ LVM_EXTRACT_PATH_MAX ];
    lvm_extract_to_extended_path( mount_point_ext, LVM_EXTRACT_PATH_MAX, mount_point );
    lvm_extract_to_extended_path( output_dir_ext, LVM_EXTRACT_PATH_MAX, output_dir );

    lvm_extract_log( L"Copying files..." );
    result = lvm_extract_copy_directory( mount_point_ext, output_dir_ext, &file_count );
    lvm_extract_log( L"Copy %s (%d files copied)",
    ...
    lvm_extract_remove_directory( mount_point_ext );
    ...
    int        *file_count )
{
    ...
    for( ;; )
    {
        ...
        {
            lvm_extract_log( "Warning: skipping unsupported file type %s", src_path );
        }
    }
    if( errno != 0 )
    {
    ...
    char *argv[] )
{
    ...
    result = lvm_extract_copy_directory( mount_point, output_dir, &file_count );
    lvm_extract_log( "Copy %s (%d files copied)",
